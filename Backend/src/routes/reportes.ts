...
interface CotizacionDetalleDTO {
  idDetalle: number;
  idCotizacion: number;
...
interface HistorialEstadoCotizacionDTO {
  id: number;
  idCotizacion: number;
...
interface CotizacionDTO {
  ...
  historialEstados: HistorialEstadoCotizacionDTO[];
}
...
interface OrdenTrabajoDTO {
  ...
  cliente: ClienteDTO | null;
  cotizacion: CotizacionOrdenDTO | null;
  instrumentos: OrdenTrabajoDetalleDTO[];
}

interface OrdenTrabajoFacturaDTO {
  idOrdenTrabajo: number;
  codigo: string;
  idCotizacion: number | null;
  idCliente: number | null;
...
interface FacturaDTO {
  ...
  estado: string;
  observacion: string | null;
  ordenTrabajo: OrdenTrabajoFacturaDTO | null;
  cliente: ClienteDTO | null;
}
...
function serializeCotizacionDetalle(row: Record<string, any>): CotizacionDetalleDTO {
  return {
...
function serializeHistorialEstadoCotizacion(row: Record<string, any>): HistorialEstadoCotizacionDTO {
  return {
    id: row.ID_HISTORIAL,
    idCotizacion: row.ID_COTIZACION_FK,
...
function serializeCotizacion(row: Record<string, any>): CotizacionDTO {
  return {
    ...
    historialEstados: (row.historial_estado_cotizacion ?? []).map(serializeHistorialEstadoCotizacion),
  };
}

function serializeCotizacionOrden(row: Record<string, any>): CotizacionOrdenDTO {
  return {
    idCotizacion: row.ID_COTIZACION,
    codigo: row.CODIGO_COTIZACION,
    idCliente: row.ID_CLIENTE_FK ?? null,
...
function serializeOrdenTrabajo(row: Record<string, any>): OrdenTrabajoDTO {
  return {
    ...
    cliente: row.clientes ? serializeCliente(row.clientes) : null,
    cotizacion: row.cotizaciones ? serializeCotizacionOrden(row.cotizaciones) : null,
    instrumentos: (row.orden_trabajo_detalles ?? []).map(serializeOrdenTrabajoDetalle),
  };
}

function serializeOrdenTrabajoFactura(row: Record<string, any>): OrdenTrabajoFacturaDTO {
  return {
    idOrdenTrabajo: row.ID_ORDEN_TRABAJO,
    codigo: row.CODIGO_OT,
    idCotizacion: row.ID_COTIZACION_FK ?? null,
...
function serializeFactura(row: Record<string, any>): FacturaDTO {
  return {
    ...
    estado: row.ESTADO,
    observacion: row.OBSERVACION ?? null,
    ordenTrabajo: row.ordenes_trabajo ? serializeOrdenTrabajoFactura(row.ordenes_trabajo) : null,
    cliente: row.clientes ? serializeCliente(row.clientes) : null,
  };
}

function serializeUsuario(row: Record<string, any>): UsuarioDTO {
  return {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        return reply.code(500).send({ success: false, error: 'Error interno del servidor al consultar los reportes' });
      }
    }
  );
}
