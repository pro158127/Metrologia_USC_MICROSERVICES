...
export const pdfRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/api/v1/pdf/upload', async (request, reply) => {
    try {
      ...
      if (!ALLOWED_MIMETYPES.includes(data.mimetype)) {
        return reply.status(415).send({ error: 'El archivo debe ser un PDF (application/pdf)' });
      }
      ...
      return reply.status(500).send({ error: 'Error interno al subir el PDF a almacenamiento' });
    }
  });

  // 2. RECUPERAR/DESCARGAR PDF POR SU RUTA URL EN MINIO (* Wildcard)
  fastify.get('/api/v1/pdf/ver/*', async (request, reply) => {
    ...
    try {
      ...
      return reply.status(500).send({ error: 'Error al procesar la descarga del PDF' });
    }
  });
